...
type Work = { full: boolean; ctx: unknown; trigger: string; settle: () => void };
type ThreadQueue = { running: boolean; pending: Work[] };

const PASSIVE_EVENTS = ['session.start', 'message_added', 'message_updated', 'agent.end'] as const;
  ...
  for (const eventName of PASSIVE_EVENTS) {
    eventApi.on(eventName, async (event, ctx) => {
      ...
      const completion = enqueue(queues, threadId, {
        full: eventName === 'session.start' || eventName === 'agent.end', ctx, trigger: eventName,
        settle: () => undefined,
      }, amp, deps);
