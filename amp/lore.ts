...
function inferLoreStateDirFromAmpConfig(home: string, env: NodeJS.ProcessEnv): string | null {
  ...
  const configHome =
    xdgConfigHome && path.isAbsolute(xdgConfigHome) ? xdgConfigHome : path.join(home, ".config");
  return inferLoreStateDirFromAmpPluginUrl(
    pathToFileURL(path.join(configHome, "amp", "plugins", "lore.ts")).href,
  );
}
...
export function configureLoreStateDirForInstalledAmpPlugin(importMetaUrl: string): void {
  ...
  const inferred =
    inferLoreStateDirFromAmpPluginUrl(importMetaUrl) ??
...
export async function shareActiveThread(
  ...
  try {
    const result = await shareAmpThread({ threadId: ctx.thread?.id }, deps);
    ...
    if (threadUrl) {
      ...
      details.push(
        copiedToClipboard
          ? "Copied Lore URL to clipboard."
          : "Could not copy Lore URL to clipboard.",
      );
...
async function copyThreadUrlToClipboard(
  ctx: PluginCommandContext,
  threadUrl: string,
): Promise<boolean> {
  try {
...
async function showCopyableThreadUrl(
  ctx: PluginCommandContext,
  threadUrl: string,
): Promise<string[]> {
  try {
...
async function appendShareUrlToThread(
  ctx: PluginCommandContext,
  threadUrl: string,
