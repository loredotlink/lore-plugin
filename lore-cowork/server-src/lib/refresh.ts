 ...
 *   `/oauth/token`, and the SECOND refresh would invalidate the first
 *   (the cloud rotates refresh tokens, so one refresh-token can only
 *   be redeemed once). The second tool call would then succeed but
...
/**
 * OAuth client identifier the cloud expects for this plugin. The cloud
 * validates this on `/oauth/token` requests as part of the device-flow
 * client registry; it is not a secret.
 */
...
async function refreshAndPersist(
  ...
  const url = `${cloudBaseUrl()}/oauth/token`;
