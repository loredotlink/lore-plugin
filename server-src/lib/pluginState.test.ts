...
describe('readPluginState', () => {
  ...
  test('backward-compat: file from before consent field was added fills in default', async () => {
    const p = pluginStateFilePath(home);
    const parent = path.dirname(p);
    fs.mkdirSync(parent, { recursive: true });
    // Write a file as it would have been before consent field existed
    fs.writeFileSync(
      p,
      JSON.stringify({ share_count: 2, watcher_prompt_dismissed: false }),
    );
    const got = await readPluginState(home);
    expect(got).toEqual({
      share_count: 2,
  ...
  test('schema rejects a file missing watcher_prompt_dismissed (required)', async () => {
    const p = pluginStateFilePath(home);
    const parent = path.dirname(p);
    fs.mkdirSync(parent, { recursive: true });
    fs.writeFileSync(p, JSON.stringify({ share_count: 0, consent: 'unconsented' }));
    await expect(readPluginState(home)).rejects.toThrow('schema validation');
  });
...
describe('writePluginState', () => {
  ...
  test('round-trips state with consent=declined', async () => {
    const state: PluginState = {
      share_count: 3,
      watcher_prompt_dismissed: false,
      consent: 'declined',
  ...
  test('round-trips state with consent=consented', async () => {
    const state: PluginState = {
      share_count: 1,
      watcher_prompt_dismissed: false,
      consent: 'consented',
  ...
  test('read is idempotent — two reads return same value without writes in between', async () => {
    await writePluginState(
      { share_count: 3, watcher_prompt_dismissed: false, consent: 'unconsented' },
      home,
    );
