...
import { writeTokens, readTokens, type Tokens } from "./auth/store";
...
function rpcError(id: string, err: { code?: number; message?: string; data?: unknown }): Response {
...
function makeRetryFetch(opts: {
  ...
  const fetchImpl = (async (url: string, init?: RequestInit): Promise<Response> => {
    ...
    if (s === MCP_PROXY_URL) {
      ...
      const status = opts.mcpStatuses[idx] ?? opts.mcpStatuses[opts.mcpStatuses.length - 1] ?? 200;
      if (status === 200) {
        return rpcSuccess(
...
describe("callCloudTool", () => {
  ...
  test("AuthRequiredError from getValidAccessToken propagates and fetch is never called", async () => {
    ...
    const { fetchImpl, calls } = captureFetch(() => jsonResponse({ shouldNotHappen: true }));
    await expect(callCloudTool("share_session", {}, { fetchImpl, home })).rejects.toBeInstanceOf(
      AuthRequiredError,
    );
  ...
  test('non-JSON 200 body: throws "cloud response was not valid JSON-RPC"', async () => {
    ...
    await expect(callCloudTool("share_session", {}, { fetchImpl, home })).rejects.toThrow(
      "cloud response was not valid JSON-RPC",
    );
  });

  test('JSON 200 body that is not a JSON-RPC envelope: throws "cloud response was not valid JSON-RPC"', async () => {
    await writeTokens(validTokens(), home);
    const { fetchImpl } = captureFetch(() => jsonResponse({ totally: "unrelated" }));
    await expect(callCloudTool("share_session", {}, { fetchImpl, home })).rejects.toThrow(
      "cloud response was not valid JSON-RPC",
    );
  });
  ...
  test("raw JSON-RPC result without the MCP content envelope is rejected", async () => {
    ...
    await expect(callCloudTool("share_session", {}, { fetchImpl, home })).rejects.toThrow(
      "cloud response was not a valid MCP tool result",
    );
  });
