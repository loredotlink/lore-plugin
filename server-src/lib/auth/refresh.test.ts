...
import { getValidAccessToken, forceRefreshAccessToken, __resetInFlightForTests } from "./refresh";
...
describe("getValidAccessToken", () => {
  test("externally-managed mode returns the desktop token slot without network calls", async () => {
    ...
    await writeClientTokens(
      stateDir(home),
      "desktop",
      freshTokens({ access_token: "desktop-access" }),
    );
    let calls = 0;
  ...
  test("externally-managed mode does not refresh, delete, or mutate token slots", async () => {
    ...
    const pluginTokens = expiredTokens({
      access_token: "plugin-expired",
  ...
  test("externally-managed mode throws AuthRequiredError when the desktop slot is absent", async () => {
    ...
    await expect(getValidAccessToken({ now, fetchImpl, home })).rejects.toBeInstanceOf(
      AuthRequiredError,
    );
    expect(calls).toBe(0);
  ...
  test("non-invalid_grant 4xx error is thrown verbatim and tokens are preserved", async () => {
    ...
    await expect(getValidAccessToken({ now, fetchImpl, home })).rejects.not.toBeInstanceOf(
      AuthRequiredError,
    );
  ...
  test("network error on token endpoint is thrown verbatim and tokens are preserved", async () => {
    ...
    await expect(getValidAccessToken({ now, fetchImpl, home })).rejects.toThrow("ECONNREFUSED");
    expect(await readTokens(home)).toEqual(tokens);
  });
  ...
  test("discovery failure (PRM 503) propagates without deleting the tokens file", async () => {
    ...
    await expect(getValidAccessToken({ now, fetchImpl, home })).rejects.toThrow();
  ...
  test("mutex error path: 10 concurrent calls all reject with AuthRequiredError; tokens deleted once; inFlight is cleared", async () => {
    ...
    await expect(getValidAccessToken({ now, fetchImpl: nextFetch, home })).rejects.toBeInstanceOf(
      AuthRequiredError,
    );
...
describe("Upload API key capability isolation", () => {
  ...
  test("does not use the LORE_API_KEY environment value as an MCP bearer", async () => {
    ...
    try {
      await expect(getValidAccessToken({ home, fetchImpl: noFetch, now })).rejects.toBeInstanceOf(
        AuthRequiredError,
      );
    } finally {
    ...
    try {
      await expect(
        forceRefreshAccessToken({
          previousAccessToken: "lore_uak_env",
          home,
          fetchImpl: noFetch,
          now,
        }),
