...
function resultText(
  result: Awaited<ReturnType<ReturnType<typeof toAmpToolDefinition>["execute"]>>,
): string {
...
describe("toAmpToolDefinition", () => {
  ...
  test("renders unsupported MCP content blocks as valid Amp text content", async () => {
    ...
    expect(result).toEqual([
      {
        type: "text",
        text: JSON.stringify({ type: "image", data: "abc123", mimeType: "image/png" }),
      },
      {
        type: "text",
        text: JSON.stringify({ type: "resource", resource: { uri: "lore://thread/1" } }),
      },
    ]);
  });
});
