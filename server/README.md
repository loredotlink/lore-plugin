...
```bash
bash packages/lore-plugin/scripts/build.sh
```

runs from the package root regardless of caller cwd, then
runs:

```bash
bun build --compile --target=bun-darwin-arm64 \
  server-src/index.ts --outfile server/lore-mcp
```

Source for the server lives in `server-src/`. See
...
`server-src/**` changes. CI enforces this by
running the build script and failing the job if
`git diff --exit-code packages/lore-plugin/server/lore-mcp` shows
unstaged changes.

If you forget: rebase, run `bash packages/lore-plugin/scripts/build.sh`, and
...
`git add packages/lore-plugin/server/lore-mcp` can OOM under sandboxed
...
git -c core.bigFileThreshold=1m -c core.compression=0 \
  add packages/lore-plugin/server/lore-mcp
```
