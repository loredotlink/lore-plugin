...
`lore/server/lore-mcp` — so the binary lives here, it's
...
```bash
bash lore/scripts/build.sh
```

The script `cd`s into `lore/` regardless of caller cwd, then
runs:
...
Source for the server lives in `lore/server-src/`. See
...
`lore/server-src/**` changes. CI (Task 9) enforces this by
running the build script and failing the job if
`git diff --exit-code lore/server/lore-mcp` shows
unstaged changes.

If you forget: rebase, run `bash lore/scripts/build.sh`, and
...
`git add lore/server/lore-mcp` can OOM under sandboxed
