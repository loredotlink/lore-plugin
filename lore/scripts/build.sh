...
# `lore/server/lore-mcp`. Committing the compiled binary
...
#
# Usage:
#   bash lore/scripts/build.sh           # from repo root
#   bash scripts/build.sh                       # from lore/
#   ./lore/scripts/build.sh              # executable, from repo root
#
# The script `cd`s to `lore/` regardless of caller cwd so
# relative paths in the bun invocation resolve predictably.

set -euo pipefail

# Resolve the directory of this script, then cd to its parent
# (lore/). Robust against symlinks and arbitrary caller cwd.
