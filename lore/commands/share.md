...
tanagram lore export --session-id ${CLAUDE_SESSION_ID} --visibility workspace $ARGUMENTS
```
...
If the command fails with "not logged in", tell the user to run `tanagram login`. If `npm` is missing, tell them to install Node.js 18+ from https://nodejs.org.
