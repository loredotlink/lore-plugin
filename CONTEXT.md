...
Capture setup belongs outside the plugin: the desktop app owns its capture process and configuration UI. The plugin does not manage capture operations.
...
Configuration and lifecycle operations for automatic capture. Desktop owns these for its capture process. The plugin owns none of them.
_Avoid_: describing the plugin as a capture client or configuration surface.

**Consumption**:
The authenticated cloud operations that list, get, fork, and search threads. The plugin serves these directly.
